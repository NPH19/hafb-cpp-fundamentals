...
int main() {
  ...
  commute_file_out.open("../file commute.csv");
  ...
  commute_file_out<<"commute minutes, commute miles"<<std::endl;
  for(int index=0; index<kMaxSize; ++index){
    commute_file_out<<commute_minutes[index]<<","<<commute_miles[index]<<std::endl;
  }
